<?php
     ...
     * Time: 4:26 PM
     */
    function getPeriodPrecedingCharacters($theString)
    {

        $theVal = '';
        $positionOfPeriod = strpos($theString, '.');
        $precedingPeriod = substr($theString, $positionOfPeriod-3, 4);
        $theNextString = substr($theString, 0, $positionOfPeriod);
        if (strpos($precedingPeriod, 'Dr.') !== false || strpos($precedingPeriod, 'Mrs.') !== false || strpos($precedingPeriod, 'Mr.') !== false || strpos($precedingPeriod, 'Ms.') !== false || strpos($precedingPeriod, 'Ph.') !== false || strpos($precedingPeriod, 'h.D') !== false) {
            $theVal = 'true';
        }

        return $theVal;
    }
    function render_the_blogs($theRss)
    {
        ob_start();
        $theStuff = $theRss;
        ?>
